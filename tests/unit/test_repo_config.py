...
from ca_bhfuil.utils.paths import url_to_path
...
class TestRepositoryConfig:
    ...
    def test_basic_repository_config(self):
        ...
        assert config.source["url"] == "https://github.com/test/repo.git"
        assert config.url_path == "github.com/test/repo"
        assert isinstance(config.branches, BranchConfig)
    ...
    def test_repository_paths(self):
        ...
        assert config.url_path == "github.com/torvalds/linux"
        assert str(config.repo_path).endswith("cache/ca-bhfuil/repos/github.com/torvalds/linux")
        assert str(config.state_path).endswith("state/ca-bhfuil/github.com/torvalds/linux")
    ...
    def test_ssh_url_conversion(self):
        ...
        assert config.url_path == "gitlab.example.com/group/project"
    ...
    def test_complex_repository_config(self):
        ...
        assert config.auth_key == "github-token"
        assert config.branches.patterns == ["main", "stable/*"]
        assert config.branches.exclude_patterns == ["experimental/*"]
        assert config.branches.max_branches == 50
...
class TestBranchConfig:
    ...
    def test_valid_patterns(self):
        ...
        config = BranchConfig(patterns=["main", "stable/*", "feature-*"])
        assert config.patterns == ["main", "stable/*", "feature-*"]
    ...
    def test_default_patterns(self):
        ...
        assert config.max_branches == 100

    def test_invalid_patterns(self):
        """Test invalid pattern validation."""
        with pytest.raises(ValueError, match="Invalid pattern"):
            BranchConfig(patterns=["[invalid"])
...
class TestConfigManager:
    ...
    def test_load_repository_configuration(self, config_manager):
        ...
        assert config.repos[1].name == "another-repo"
    ...
    def test_get_repository_by_url(self, config_manager):
        ...
        test_config = {
            ...
            "repos": [
                {
                    "name": "os-vif",
                    "source": {"url": "https://opendev.org/openstack/os-vif.git", "type": "git"},
                }
            ],
        }
    ...
    def test_configuration_validation(self, config_manager):
        ...
        test_config = {
            ...
            "repos": [
                {
                    "name": "duplicate-name",
                    "source": {"url": "https://github.com/test/repo1.git", "type": "git"},
                },
                {
                    "name": "duplicate-name",  # Duplicate name
                    "source": {"url": "https://github.com/test/repo2.git", "type": "git"},
                },
                {
                    "name": "missing-auth",
                    "source": {"url": "https://github.com/test/repo3.git", "type": "git"},
...
class TestRealWorldConfiguration:
    ...
    def test_repository_paths_generation(self, real_world_config):
        ...
        config = real_world_config.load_configuration()
