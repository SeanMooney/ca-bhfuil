...
from ..config import AuthMethod, ConfigManager, RepositoryConfig
from ...utils.paths import get_repo_paths
...
class RepositoryCloner:
    ...
    def _perform_clone(
        self,
        repo_config: RepositoryConfig,
        progress_callback: Optional[Callable[[CloneProgress], None]],
        start_time: float,
    ) -> CloneResult:
    ...
    def _setup_authentication(self, repo_config: RepositoryConfig) -> dict[str, Any]:
    ...
    def _setup_ssh_auth(self, auth_method: AuthMethod) -> dict[str, Any]:
        """Set up SSH key authentication."""
        if not auth_method.ssh_key_path:
            return {}
        
        ssh_key_path = Path(auth_method.ssh_key_path).expanduser()
    ...
    def _setup_token_auth(self, auth_method: AuthMethod) -> dict[str, Any]:
    ...
    def validate_clone(self, repo_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                checks["has_objects"] = objects_dir.exists() and any(objects_dir.iterdir())
    ...
    def remove_repository(self, repo_config: RepositoryConfig) -> bool:
        ...
        try:
            ...
            if lock_file.exists():
                logger.error(f"Cannot remove repository, clone in progress: {repo_path}")
                return False
...
def validate_repository_by_path(repo_path: Path) -> dict[str, Any]:
